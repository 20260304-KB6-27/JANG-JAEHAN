...
// const addPost = () => {
//   // 빈 문자열 예외처리
//   if (new_post.value.title === '') {
//     alert('제목을 입력해주세요.');
//     return;
//   }

//   if (new_post.value.content === '') {
//     alert('내용을 입력해주세요.');
//     return;
//   }

//   posts.value.push({
//     no: posts.value[0].no + 1,
...
const updatePost = (updatedPost) => {
  ...
  const index = posts.value.findIndex((item) => item.no === updatedPost.no);
  if (index === -1) return;
...
const deletePost = (no) => {
  if (!confirm('정말로 삭제하시겠습니까?')) {
    ...
    // findIndex(콜백) : 콜백함수가 true를 반환하는 첫 번째 요소의 인덱스를 반환
    let index = posts.value.findIndex((item) => no === item.no);

    // splice(인덱스, 삭제할 요소의 갯수) : 인덱스로부터 지정한 갯수만큼 배열의 요소를 삭제
