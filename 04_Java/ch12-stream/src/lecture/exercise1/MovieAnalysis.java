...
public class MovieAnalysis {
    public static void main(String[] args) {
        ...
        // 결과 : 약 8.7
        double sfAverageRating = movies.stream()
                .filter(movie -> movie.getGenre().equals("SF"))
                .mapToDouble(Movie::getRating)
                .average()
                .orElse(0.0);

        System.out.println("1번 결과: " + sfAverageRating);


        // 2. 2010년 이후 개봉된 영화를 평점 내림차순으로 정렬하여 제목 리스트를 반환하기
        // 결과 : [인셉션, 기생충, 인터스텔라, 어바웃타임, 라라랜드, 어벤져스]
        List<String> titlesAfter2010SortedByRating = movies.stream()
                .filter(movie -> movie.getReleaseYear() >= 2010)
                .sorted((m1, m2) -> Double.compare(m2.getRating(), m1.getRating()))
                .map(Movie::getTitle)
                .collect(Collectors.toList());

        System.out.println("2번 결과: " + titlesAfter2010SortedByRating);


        // 3. 가장 최근에 개봉된 영화 3개의 제목을 반환하기
        //  결과 : [기생충, 라라랜드, 인터스텔라]
        List<String> recentTop3Titles = movies.stream()
                .sorted((m1, m2) -> Integer.compare(m2.getReleaseYear(), m1.getReleaseYear()))
                .limit(3)
                .map(Movie::getTitle)
                .collect(Collectors.toList());

        System.out.println("3번 결과: " + recentTop3Titles);
        ...
        System.out.println("4번 결과: " + genreCountMap);


        // 5. 평점이 8.5 이상인 영화들의 장르를 중복 없이 알파벳 순으로 정렬하여 반환하기
        // 결과 : [SF, 드라마, 액션]
        List<String> genresOfHighRatedMovies = movies.stream()
        ...
        System.out.println("5번 결과: " + genresOfHighRatedMovies);
    }
}