...
public class OrderAnalysis {
    public static void main(String[] args) {
        ...
        // 결과: [1, 4]
        List<Integer> orderIdsOverMillion = orders.stream()
                .filter(order -> order.getTotalAmount() >= 1000000)
                .map(Order::getOrderId)
                .collect(Collectors.toList());

        System.out.println("1번 결과: " + orderIdsOverMillion);
        ...
        System.out.println("2번 결과: " + totalAmountByCustomer);
        ...
        System.out.println("3번 결과: " + mostPurchasedCategory);


        // 4. 2023년 5월 이후의 주문에서 구매된 모든 제품의 이름을 중복 없이 반환하기
        // 결과: [자바 책, 태블릿, 노트북, 스마트폰, 충전기]
        List<String> productNamesAfterMay = orders.stream()
                .filter(order -> !order.getOrderDate().isBefore(LocalDate.of(2023, 5, 1)))
                .flatMap(order -> order.getProducts().stream())
                .map(Product::getName)
                .distinct()
                .collect(Collectors.toList());

        System.out.println("4번 결과: " + productNamesAfterMay);
        ...
        System.out.println("5번 결과: " + averagePriceByCategory);
        ...
        System.out.println("6번 결과: " + totalAmountByDate);
    }
}