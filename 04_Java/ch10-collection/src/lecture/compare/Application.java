...
//        Set<Student> student2 = new TreeSet<>(new StudentComparatorByName());

        // 익명객체 (1회용 객체)
//        Set<Student> student2 = new TreeSet<>(new Comparator<Student>() {
//            @Override
...
//        });
        ...
        // 람다 Comparator 구현
        Set<Student> student2 = new TreeSet<>(
                (Student o1, Student o2) -> o1.getName().compareTo(o2.getName())
        );

        student2.addAll(students); // 기존 데이터 옮기기
        ...
        for (Student s : list) {
            ...
            teamCount.put(
                    s.getTeam(),
                    // key에 해당하는 값이 있으면 값 반환, 아니면 기본값 반환
        ...
        // 만든 기준대로 정렬
        list.sort(new StudentComparatorByTeamsSize(teamCount));
        ...
        // Map - Iterator 사용 불가능 cuz, 컬렉션 구현 X => So, Set으로 변환 후 사용
